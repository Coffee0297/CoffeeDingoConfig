...
public class DbcSignalCodec
{
        ...
        bool isFloat = false)
    {
        ...
        {
            // Motorola/Big Endian: start bit is the MSB of the signal. In DBC "sawtooth"
            // numbering the absolute bit = byte*8 + bitInByte (0=LSB..7=MSB), so the MSB's
            // position within its byte is (startBit % 8) — NOT 7-(startBit%8).
            int startByte = startBit / 8;
            int startBitInByte = startBit % 8; // Bit position within byte (7=MSB, 0=LSB)
        ...
        bool isFloat = false)
    {
        ...
        {
            // Motorola/Big Endian: start bit is the MSB (in-byte position = startBit % 8).
            int startByte = startBit / 8;
            int startBitInByte = startBit % 8;
            int bitsRemaining = length;
