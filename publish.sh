...
echo ""
echo "Building Svelte SPA (web/clientapp -> web/wwwroot)..."
( cd web/clientapp && npm ci && npm run build ) || { echo "SPA build failed"; exit 1; }
echo "[OK] SPA build complete"

# Common publish arguments
PROJECT="web/web.csproj"
CONFIG="Release"
# Single self-contained launcher per platform (SPA + device defs embedded → runs standalone).
PUBLISH_ARGS="--self-contained true -p:PublishSingleFile=true -p:EnableCompressionInSingleFile=true -p:DebugType=none"
...
dotnet publish "$PROJECT" -c "$CONFIG" -r linux-arm $PUBLISH_ARGS -o "$OUTPUT_DIR/linux-arm32"
if [ $? -eq 0 ]; then
...
echo ""
echo "========================================="
echo "Pruning to the single launcher exe..."
# Everything is inside the exe; drop loose byproducts so each zip is just the launcher.
for d in "$OUTPUT_DIR"/*/; do
    find "$d" -mindepth 1 ! -name 'dingoConfig' ! -name 'dingoConfig.exe' -prune -exec rm -rf {} + 2>/dev/null
done
